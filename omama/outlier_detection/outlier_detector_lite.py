...
class OutlierDetectorLite:
    ...
    def extract_data(self, results_dict, variable='jaccard_score', sort=True,
                     sort_by='mean'):
    ...
    """
        ...
        for algo in results_dict.keys():
            ...
            try:
                ...
                print('scores', scores)
                print(
                    f'algo: {algo}, norm: {norm}, feat: {feat}, dataset: {dataset}')
    ...
    def results_to_latex_alt_style(self,
                                   dataset_paths: list,
                                   variable='jaccard_score',
                                   display=True,
                                   d=4) -> str:
        ...
        for i in range(0, len(values[0].keys())):
            ...
            for j in range(len(values)):
                if counter == 0:
                    ...
                    if values[j][algo]['std'] < THRESHOLD or values[j][algo]['std'] == np.nan:
                        latex += '%s & %s + %s & %.4f &' % (
                            algo, norm, feat, values[j][algo]['mean'])
                    else:
                        latex += '%s & %s + %s & $%.4f\pm%.4f$ &' % (
                            algo, norm, feat, values[j][algo]['mean'], values[j][algo]['std'])
                else:
                    norm = self.norm_abbr(values[j][algo]['norm'])
                    feat = self.feature_abbr(values[j][algo]['feat'])
                    if values[j][algo]['std'] < THRESHOLD or values[j][algo]['std'] == np.nan:
                        latex += '%s + %s & %.4f &' % (
                            norm, feat, values[j][algo]['mean'])
                    else:
                        latex += '%s + %s & $%.4f\pm%.4f$ &' % (
                            norm, feat, values[j][algo]['mean'], values[j][algo]['std'])
                counter += 1
    ...
    def norm_abbr(self, norm):
        if norm == 'gaussian' or norm == 'Gaussian':
            ...
            return 'M'
        elif norm == 'minmax' or norm == 'MinMax' or norm == 'min-max' or norm == 'Min-Max':
            return 'MM'
        elif norm == 'robust' or norm == 'Robust':
            return 'R'
        elif norm == 'zscore' or norm == 'ZScore' or norm == 'z-score' or norm == 'Z-Score':
            return 'Z'
        else:
            return norm
    ...
    def get_best_results_for_each_algorithm(self,
        ...
        best_results = sorted(best_results.items(),
        ...
        if display:
            print(
                'Dataset & Algorithm & Norm. + Feature & %s' % variable.replace(
                    '_', ' ').title())
            for algo in best_results.keys():
                print(
                    '%s & %s & %s + %s & %.4f' % (best_results[algo]['dataset'],
                                                  algo,
    ...
    def get_best_results_per_dataset(self,
                                     dataset_paths: list = None,
                                     cache_root=OUTPUTDIR,
                                     variable='roc_auc',
                                     display=True) -> dict:
        ...
        for i in range(len(dataset_names)):
            for algo in values[i].keys():
                if dataset_names[i] not in best_results.keys():
                    ...
                    if values[i][algo]['mean'] > best_results[dataset_names[i]][
                        'mean']:
                        best_results[dataset_names[i]] = values[i][algo]
                        best_alg = algo
    ...
    def print_all_experiments_results(self,
                                      dataset_paths: list = None,
                                      cache_root=OUTPUTDIR,
                                      variable='roc_auc',
                                      to_latex=False,
...
def sort_dict_by_value_v2(d: dict,
                          reverse=True,
                          variable='tp/totalbad',
                          display=True) -> dict:
    """ Sorts a dictionary by its values
