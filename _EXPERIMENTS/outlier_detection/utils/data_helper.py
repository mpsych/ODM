...
class DataHelper(object):
    ...
    @staticmethod
    def get2D(
        N=1, dataloader=None, cancer=None, randomize=False, config_num=2, timing=False
    ):
    ...
    def view(
        ...
        if histogram:
            if normalize:
                ...
                img = Normalize.minmax(img)
            img_hist = Features.histogram(img)
            plt.hist(img_hist)
        else:
            # display the image
    ...
    def show_histogram(image, norm_type="min-max"):
        ...
        # normalize the image
        img = Normalize.get_norm(pixels, norm_type=norm_type)
