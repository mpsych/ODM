...
def vae(data_x):
    ...
    if isinstance(data_x, np.ndarray):
        ...
        for i in range(len(data_x)):
            if len(data_x[i]) == 1:
                data_x[i] = np.pad(
                    data_x[i], (0, len(data_x[0]) - 1), "constant")
            if len(data_x[i]) == 3:
                data_x[i] = np.pad(
                    data_x[i], (0, len(data_x[0]) - 3), "constant")
    ...
    hidden_activation = config.get(
        'HYPERPARAMS', 'hidden_activation', fallback='relu')
    ...
    l2_regularizer = config.getfloat(
        'HYPERPARAMS', 'l2_regularizer', fallback=0.1)
    validation_size = config.getfloat(
        'HYPERPARAMS', 'validation_size', fallback=0.1)
    preprocessing = config.getboolean(
        'HYPERPARAMS', 'preprocessing', fallback=True)
    verbose = config.getint('HYPERPARAMS', 'verbose', fallback=1)
    contamination = config.getfloat(
        'HYPERPARAMS', 'contamination', fallback=0.1)
    gamma = config.getfloat('HYPERPARAMS', 'gamma', fallback=1.0)
    capacity = config.getfloat('HYPERPARAMS', 'capacity', fallback=0.0)
    random_state = ast.literal_eval(config.get(
        'HYPERPARAMS', 'random_state', fallback='None'))
    encoder_neurons = ast.literal_eval(config.get(
        'HYPERPARAMS', 'encoder_neurons', fallback='None'))
    decoder_neurons = ast.literal_eval(config.get(
        'HYPERPARAMS', 'decoder_neurons', fallback='None'))
